...
Hệ thống tiếp nhận đầu vào là các tệp JSON được sinh ra từ mô-đun ASR. Do đặc thù của dữ liệu hội thoại tự nhiên, thông tin đầu vào thường gặp các vấn đề như thiếu nhãn người nói hoặc định dạng không đồng nhất. Để đảm bảo tính toàn vẹn dữ liệu, quy trình chuẩn hóa được áp dụng trong phương thức \texttt{\_load\_memory\_kb} với các tiêu chí sau:

\begin{itemize}
    \item \textbf{Phân đoạn văn bản (Chunking):} Mỗi câu thoại dài được chia nhỏ thành các đoạn (\textit{chunks}) có độ dài tối đa 40 từ với độ chồng lấp (\textit{overlap}) là 10 từ. Việc này giúp vector đại diện tập trung hơn vào các ý chính và tránh hiện tượng mất mát thông tin tại ranh giới các đoạn.
