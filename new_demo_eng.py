...
def load_real_audio(path: Path):
    if not path.exists():
        ...
        if mp4_path.exists():
            print(f"[INFO] Converting MP4 → WAV: {mp4_path.name}")
            path = Path(mp4_to_wav(mp4_path))
        else:
...
def postprocess_diarization(client, diarized_segments):
    ...
    system_prompt = """
Task: Fix diarization turn boundaries, speaker attribution, and clean text.

You may:
- Move words or sentences between adjacent segments when boundary or speaker attribution is clearly wrong.
- Reassign text to a different existing speaker if conversational flow makes it obvious.
- Correct spelling, casing, and punctuation (autocorrect only; no paraphrasing).

You must:
- Preserve all words and their meaning (no additions or deletions).
- Do not create, delete, or rename speaker IDs.
- Keep timestamps monotonic and consistent with the reordered text.
- Use any provided previous segments only as context; do not output them.
...
for idx, audio_path in enumerate(audio_files):
    ...
    print(f"[{idx}] Audio duration: {h:02d}:{m:02d}:{s:02d}")
    ...
    # ---------- NO DER (no ground truth) ----------
    # der, miss_rate, fa_rate, conf_rate = der_benchmark_full(...)
    # avg_miss += miss_rate
    # avg_fa += fa_rate
    # avg_conf += conf_rate
    ...
    final_json = concat_json_arrays(raw)

    save_transcriptions_json(
        final_json,
        Path("transcriptions") / f"final_transcriptions_{audio_path.stem}.json"
    )
